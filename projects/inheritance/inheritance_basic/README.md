...
3. 定义 Person 类, 包含以下成员。请注意封装性(数据成员私有，通过方法访问).
...
5. 定义 Student 类为 Person 类的子类，并实现以下内容：
...
6. 定义 Teacher 类为 Person 类的子类，并实现以下内容：
...
7. 请编写程序验证 Teacher, Student 类中 talk() 方法是动态绑定的：
...
11. 定义 ElderTeacher 类为 Teacher 类的子类，实现以下内容：
...
12. 假设我们不再希望程序中出现普通的 "Person" 对象（人必须有具体的身份），编写程序实现以下内容：
...
13. 在 Person 类中定义一个方法 `public final void breathe()`，输出 "Breathing..."。然后尝试在 Student 类中重写 breathe() 方法，观察编译器报错信息，并解释原因。

14. 假设在我们的系统中，每个人都可以有一个“同伴”(Partner)。先修改以下内容：
